...
class EFQueryLensUrlOpener : UrlOpener() {
    ...
    override fun openUrl(
        ...
        // Intercept localhost action-server URLs generated by the LSP hover popup.
        // JCEF calls BrowserLauncher.open() for http:// links, which invokes this
        // UrlOpener before actually opening a browser tab.  By handling the URL here
        // and returning true we perform the IDE action without any browser window.
        if (url.startsWith("http://127.0.0.1", ignoreCase = true)) {
            return handleActionUrl(url, project)
        }
        ...
        // Legacy efquerylens:// scheme (kept for completeness / future use)
    ...
    /**
     * Handles `http://127.0.0.1:{port}/efquerylens/action?type=…&uri=…&line=…&character=…`
     * URLs without opening a browser tab.  Returns true only when the URL matches our
     * action-server path so unrelated localhost URLs still fall through to the browser.
     */
    private fun handleActionUrl(
        ...
        thisLogger().info("[EFQueryLens] UrlOpener intercepted action type=$type line=$line char=$character")
