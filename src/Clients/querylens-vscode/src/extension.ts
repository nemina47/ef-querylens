...
export function activate(context: ExtensionContext) {
        ...
        middleware: {
            provideCodeLenses: async (document, token, next) => {
                ...
                if (settings.debugLogsEnabled) {
                    ...
                    console.log(`[QueryLens] provideCodeLenses uri=${document.uri.toString()} count=${count} elapsedMs=${elapsed}`);
                }
            ...
            provideHover: async (document, position, token, next) => {
                ...
                return enableTrustedHoverCommands(hover, ['querylens.copySql', 'querylens.showSql']);
            }
        },
        synchronize: {
    ...
    const showSqlFromCursorCommand = commands.registerCommand(
        'querylens.showSqlFromCursor',
        async () => {
            const location = getActiveEditorLocation();
            if (!location) {
                window.showInformationMessage('QueryLens: open a C# file and place the cursor on a query first.');
                return;
            }
...
async function showSqlFromLens(
    ...
    if (includeCopyAction) {
        const action = await window.showInformationMessage('QueryLens SQL ready', 'Copy SQL');
        if (action === 'Copy SQL') {
            await env.clipboard.writeText(sqlText);
            window.showInformationMessage('QueryLens: SQL copied to clipboard.');
        }
    }
}
...
async function getSqlForLens(
    ...
    const uri = parseUri(uriInput);
    if (!uri) {
        window.showWarningMessage('QueryLens: unable to resolve document URI for SQL preview.');
        return null;
    }
    ...
    try {
        ...
        if (!hoverText) {
            window.showInformationMessage('QueryLens: no SQL preview available at this location.');
            return null;
        }
        ...
        window.showErrorMessage(`QueryLens: failed to retrieve SQL preview. ${message}`);
        return null;
    }
}
...
function readSettings(): {
    ...
    const config = workspace.getConfiguration('querylens');
